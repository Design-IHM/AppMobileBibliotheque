...
def similar_books():
    try:
        ...
        # Construct the books list
        books_list = []
        for book in books:
            book_data = book.to_dict()
            if 'name' in book_data:  # Vérifier que le livre a un nom
        ...
        # Trouver le livre de base
        base_book = next((book for book in books_list if book['name'].lower() == book_title.lower()), None)
        if not base_book:
            return jsonify({"books": []}), 200

        # Extraire les descriptions et les titres
        descriptions = []
        for book in books_list:
            desc = book.get('desc', '')
            if not desc:  # Si pas de description, utiliser le titre
                desc = book.get('name', '')
        ...
        try:
            ...
            # Trouver l'index du livre de base
            base_index = next(i for i, book in enumerate(books_list) if book['name'].lower() == book_title.lower())
            
            # Calculer les scores de similarité
            similarity_scores = list(enumerate(similarity_matrix[base_index]))
            similarity_scores = sorted(similarity_scores, key=lambda x: x[1], reverse=True)

            # Prendre les 5 livres les plus similaires (excluant le livre de base)
            similar_books = []
            for idx, score in similarity_scores[1:6]:  # Skip the first one (it's the base book)
                book = books_list[idx]
                book['similarity_score'] = float(score)
        ...
        print(f"Erreur dans similar_books: {str(e)}")
...
def calculate_user_similarity(user1_data, user2_data):
    ...
    try:
        ...
        dept1 = user1_data.get('departement', '').lower()
        dept2 = user2_data.get('departement', '').lower()
        if dept1 and dept2 and dept1 == dept2:
            similarity_score += 40.0
        
        # 2. Même niveau d'études (20 points)
        niveau1 = str(user1_data.get('niveau', '')).lower()
        niveau2 = str(user2_data.get('niveau', '')).lower()
        if niveau1 and niveau2 and niveau1 == niveau2:
            similarity_score += 20.0
        
        # 3. Historique de consultation récent (25 points)
        recent_docs1 = user1_data.get('docRecent', [])
        recent_docs2 = user2_data.get('docRecent', [])
...
def get_similar_users_recommendations(user_email):
    ...
    try:
        print(f"Recherche de recommandations pour l'utilisateur: {user_email}")
        ...
        print(f"Données utilisateur trouvées: {user_data.get('name', 'Nom non trouvé')}")
        ...
        for other_user in all_users:
            if other_user.id != user_email:
                ...
                # Vérifier si l'utilisateur a des documents récents
                if not other_user_data.get('docRecent'):
                    continue
                
                similarity = calculate_user_similarity(user_data, other_user_data)
                print(f"Similarité avec {other_user.id}: {similarity}")
        ...
        print(f"Nombre d'utilisateurs similaires trouvés: {len(similar_users)}")
        ...
        response_data = {
            ...
            'user_info': {
                ...
                'niveau': user_data.get('niveau', '')  # Changé de 'level' à 'niveau'
            }
        }
